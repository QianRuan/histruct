...
######Eval Arguments############################################################################################################## 
#how many sentences are etracted as the final summary, BLOCK TRIGRAM is applied by default on CNN/DailyMail
SELECT_TOP_N_SENT = 3
TEST_BATCH_SIZE = 500
ALPHA=0.95
...
DATASET = 'cnndm'
PATH = '' #root dir
DATA_PATH = PATH+'data_cnndm/data_cnndm_roberta/cnndm'
TEMP_DIR = PATH+'temp'
...
MAX_NSENT = 407 #should be set to a number that is larger than the number of input sentences
...
#the base TLM used in the summarization model, choose from ['bert-base','bert-large','roberta-base','roberta-large']
BASE_LM = 'roberta-base'

#whether the token-level hierarchical position information (tHE) is injected, default:'false'
ADD_TOK_SE = 'false'
...
MODEL_NAME = MODEL_NAME + '_FT' #on cnndm, the base TLM is finetuned by default
